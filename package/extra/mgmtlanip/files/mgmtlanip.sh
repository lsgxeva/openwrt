...
IPV4_BR_ADDRESS='192.168.111.254'
IPV4_BR_NETMASK='255.255.255.0'
INTERFACE_WAN_NAME='eth1'
INTERFACE_WAN_INDEX='0'
IPV4_WAN_ADDRESS='192.168.0.2'
IPV4_WAN_NETMASK='255.255.255.0'
...
create_mgmtlanip_config()
{
	...
	uci -q batch <<-EOF >/dev/null
		...
		set mgmtlanip.@mgmtlanip[-1].ipv4_br_address='192.168.111.254'
		set mgmtlanip.@mgmtlanip[-1].ipv4_br_netmask='255.255.255.0'
		set mgmtlanip.@mgmtlanip[-1].interface_wan_name='eth1'
		set mgmtlanip.@mgmtlanip[-1].interface_wan_index='0'
		set mgmtlanip.@mgmtlanip[-1].ipv4_wan_address='192.168.0.2'
		set mgmtlanip.@mgmtlanip[-1].ipv4_wan_netmask='255.255.255.0'
		commit mgmtlanip
	EOF
}
...
get_mgmtlanip_config()
{
	...
	INTERFACE_WAN_NAME="$( uci get mgmtlanip.@mgmtlanip[-1].interface_wan_name )"
	INTERFACE_WAN_INDEX="$( uci get mgmtlanip.@mgmtlanip[-1].interface_wan_index )"
	IPV4_WAN_ADDRESS="$( uci get mgmtlanip.@mgmtlanip[-1].ipv4_wan_address )"
	IPV4_WAN_NETMASK="$( uci get mgmtlanip.@mgmtlanip[-1].ipv4_wan_netmask )"
}
...
mgmtlanip_main()
{
	...
	[ $( ubus call network.device status "{\"name\":\"$INTERFACE_WAN_NAME\"}" | jsonfilter -q -e '@.up' ) == "true" ] &&
		_fix_mgmtlanip "$INTERFACE_WAN_NAME" "$INTERFACE_WAN_INDEX" "$IPV4_WAN_ADDRESS" "$IPV4_WAN_NETMASK" 
