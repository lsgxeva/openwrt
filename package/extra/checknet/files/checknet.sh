...
modem_dial_run()
{
	...
	# Note: Setting qmi param
	[ -d /sys/class/net/${ifname}/qmi ] && echo N > /sys/class/net/${ifname}/qmi/pass_through
	[ -d /sys/class/net/${ifname}/qmi ] && echo Y > /sys/class/net/${ifname}/qmi/raw_ip
	#
