...
static uint64_t GetTimeStamp()
{
    ...
    uint64_t host_timestamp = tm.tv_sec * (uint64_t)1000 + tm.tv_usec / 1000 + 28800000;
    ...
    if (g_is_unisoc_chip == 1)
    {
        if (cfg)
        {
            ...
            while (fgets(recv_cfg_buf, sizeof(recv_cfg_buf), fp_cfg)) {
                size_t length_cfg = strlen(recv_cfg_buf);
                if (length_cfg > sizeof(recv_cfg_buf))
                {
                    return -1;
                }
    ...
    for (i=0;i<ql_exx00u_xml_data_defult.ql_cmd_count;i++)
    {
        ...
        if (strStartsWith(ql_exx00u_xml_data_defult.ql_cmd_table[i].cmd.type, "region"))
        {
            ...
            else
            {
                ...
                snprintf(filename, sizeof(filename), "%.264s/%02x%02x%02x%02x.bin", blue_screen_logfile_dir, filename_tmp[3], filename_tmp[2], filename_tmp[1], filename_tmp[0]);
